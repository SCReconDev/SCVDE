...
void send_email(EmailSenderUtility *utility, const char *to, const char *subject, const char *body) {
    printf("Sending email to %s with subject '%s' via %s:%d with username %s and password %s\n",
           to, subject, utility->smtp_server, utility->smtp_port, utility->username, utility->password);
}
