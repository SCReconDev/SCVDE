...
typedef struct {
    char *deployment_helper_name;
    char *registry_password;
} DeploymentHelper;

DeploymentHelper *create_deployment_helper(const char *name) {
    DeploymentHelper *helper = (DeploymentHelper *)malloc(sizeof(DeploymentHelper));
    if (helper == NULL) {
        return NULL;
    }

    helper->deployment_helper_name = strdup(name);
...
void destroy_deployment_helper(DeploymentHelper *helper) {
    if (helper != NULL) {
        free(helper->deployment_helper_name);
...
int main() {
    DeploymentHelper *helper = create_deployment_helper("DeploymentHelper1");
    if (helper == NULL) {
        fprintf(stderr, "Failed to create deployment helper\n");
        return 1;
    }

    printf("Deployment Helper created with name: %s\n", helper->deployment_helper_name);
