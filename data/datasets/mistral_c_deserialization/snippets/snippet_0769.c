...
void deserialize_fixture(const char* json_str, Fixture* fixture) {
    ...
    if (!json_object_object_get_ex(parsed_json, "fixture_name", &fixture_name_obj)) {
        fprintf(stderr, "Error getting fixture_name from JSON\n");
        json_object_put(parsed_json);
        return;
    }

    if (!json_object_object_get_ex(parsed_json, "fixture_data", &fixture_data_obj)) {
        fprintf(stderr, "Error getting fixture_data from JSON\n");
        json_object_put(parsed_json);
        return;
    }
...
int main() {
    const char* json_str = "{\"fixture_name\":\"test_fixture\",\"fixture_data\":\"test_value=123\"}";
    Fixture fixture;
