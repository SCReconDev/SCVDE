...
#define ACCESS_KEY_ENV "ACCESS_KEY"
...
CloudStorageHelper *create_cloud_storage_helper() {
    ...
    char *access_key = getenv(ACCESS_KEY_ENV);
    if (access_key == NULL) {
        free(helper->url);
        free(helper);
        return NULL;
...
int main() {
    ...
    printf("Cloud Storage Helper created with URL: %s and Access Key: %s\n", helper->url, helper->access_key);
