...
void deserialize_command(const char* json_str, Command* command) {
    ...
    if (json_object_object_get_ex(parsed_json, "command_name", &command_name)) {
        command->command_name = strdup(json_object_get_string(command_name));
    }

    if (json_object_object_get_ex(parsed_json, "command_args", &command_args)) {
...
int main() {
    const char* json_str = "{\"command_name\":\"restore\",\"command_args\":\"--file example.txt\"}";
    Command command;
