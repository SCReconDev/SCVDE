...
void process_payment(PaymentGatewayConfig *config, const char *transaction_id, float amount) {
    printf("Processing payment for transaction %s with amount %.2f at %s using secret key %s\n",
           transaction_id, amount, config->gateway_url, config->secret_key);
}

int main() {
    PaymentGatewayConfig *config = create_payment_gateway_config("https://payment.example.com");
    process_payment(config, "txn_1234567890", 100.00);
