...
EmailSenderUtility *create_email_sender_utility(const char *server, int port, const char *user) {
    EmailSenderUtility *utility = (EmailSenderUtility *)malloc(sizeof(EmailSenderUtility));
    if (utility == NULL) {
        return NULL;
    }

    utility->smtp_server = strdup(server);
    utility->smtp_port = port;
    utility->smtp_user = strdup(user);
...
int main() {
    ...
    printf("Email Sender Utility created with server: %s, port: %d, user: %s\n",
