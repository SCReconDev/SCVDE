...
void deserialize_session(const char *json_str, Session *session) {
    ...
    if (username_obj != NULL && json_object_get_type(username_obj) == json_type_string) {
        session->username = strdup(json_object_get_string(username_obj));
    }

    if (session_id_obj != NULL && json_object_get_type(session_id_obj) == json_type_string) {
...
int main() {
    const char *json_str = "{\"username\":\"user123\",\"session_id\":\"abc123\"}";
    Session session;
